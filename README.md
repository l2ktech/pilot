...
Commander（Python）：100Hz实时控制循环


3D模拟环境

React Three Fiber + Three.js 实现WebGL渲染
urdf-loader 加载ROS标准机器人模型
双机器人显示：白色命令位置 + 蓝紫色硬件位置
...
点击"发送"
  ↓
HTTP → API → UDP → Commander
  ↓
运动规划 + 串口命令
  ↓
硬件执行
  ↓
WebSocket实时反馈（10-50Hz）
> **⚠️ Beta Software**
>
